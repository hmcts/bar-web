...
export class PaymentOverviewComponent implements OnInit {
  ...
  get user (): UserModel {
    ...
    const pendingApprovalCount = this.paymentOverviewService.getPaymentInstructionCount(PaymentStatus.getPayment('PA').code);
    const pendingReviewCount = this.paymentOverviewService.getPaymentInstructionCount(PaymentStatus.getPayment('A').code);
